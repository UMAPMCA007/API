...
class DashBoardController extends Controller
{
    ...
    public function activity_add(Request $request,$id)
    {
        ...
        if($activity){
            $last_add=Activity::all()->last()->pluck('created_at');
            // current date
            ...
            if($last_add_date<$current_date)
            {
                ...
                {
                    return response()->json(['error'=>'You have already added 2 activity']);
                }
            }else{
                return response()->json(['error'=>'You have already added 2 activity in a day']);
            }   
